...
function lnfZcondΩ_prep(ω; NLegendre, σ::AbstractVector{T}, μₘ, σₘ) where {T}
...
function add_derived_stats!(est::HnFresult)
	function derived_stats(; p,μ,τ,ν,pDFR,σ,μₘ,σₘ)
		...
		entropy_gain = H_Ω_Z(;p,μ,τ,ν,pDFR,σ,μₘ,σₘ) - H_Ω_Z(;p,μ,τ,ν,pDFR=[1.,0,0], σ,μₘ=[-100.],σₘ=[1.])  # H(Ω|Z) - H(Ω|Z₀)
		...
		equiv_sample_reduction = 1 - find_zero(τ_multiplier -> H_z₀(τ_multiplier) - (H_z₀(1) - entropy_gain), (0.01, 1.5))
...
@time begin
	InverseChisq(ν,τ²) = InverseGamma(ν/2, ν*τ²/2)  # https://en.wikipedia.org/wiki/Scaled_inverse_chi-squared_distribution
  penalty(; τ::Vector{T}, σ::Vector{T}, σₘ::Vector{T}, kwargs...) where {T} = 
		logpdf(InverseChisq(1,1), σ[]^2) + 
		logpdf(InverseChisq(1,1), σₘ[]^2) + 
		sum(logpdf(InverseChisq(1,1), τᵢ^2) for τᵢ ∈ τ) 
  ...
  SW = HnFestimate(df, :z, :weight_table; penalty, estname="SW")
	# SW = HnFestimate(df, :z, :weight_table; penalty, estname="SW", xform=(μ=constant_xform([0.])))  # example of parameter constraint
...
df = DataFrame(XLSX.readtable("data/Huntington-Klein et al 2025/Specification_curve_data.xlsx", "Sheet1", infer_eltypes=true)...)
