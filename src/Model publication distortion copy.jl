	...
	Threads.@threads for i ∈ eachindex(z₀)  # for each simulated study
		@inbounds begin
			...
			if abs(z₀ⱼ) > z̄  # if initial result significant, publish as is
				...
				m = round(Int, rand(Normal(μₘ[j], σₘ[j])))  # number of measurements to be taken if p-hacking
				if m ≤ 0
					z₁ⱼ = z₀ⱼ
				else
					batch = rand(Normal(z₀ⱼ, σ[]), m)  # m measurements
					absbatch = abs.(batch)
					j = findfirst(x->abs(x)==maximum(absbatch), absbatch)  # most significant of batch
					if absbatch[j] > z̄  # if significant, publish and stop
						z[i] = batch[j]
						continue
					end
				end
				r = rand(Distributions.Categorical(pDFR))
				z[i] = r==1 ? z₁ⱼ : r==2 ? NaN : z₀ⱼ  # publish final, significant result; file-drawer; publish initial, insignificant result
			end
